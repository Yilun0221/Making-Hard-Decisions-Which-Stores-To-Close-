...
params_dict = {'p_2': 'R_Percentage_of_Visits_by_brand', 'p_6':'G_Distance_between_cbg_and_store'}
...
def compute_row(row,p_2,p_6):
    ...
    return up_first/down

def huff_pearson(df, cbg, p_2,p_6):
    rate = get_rate(df, cbg)
    df_temp = df[df['A_cbg'] == cbg].drop(columns = ['A_cbg'])
    df_temp['assumed_rate'] = df_temp.apply(lambda row: compute_row(row,p_2,p_6),axis = 1)
...
def get_final_pearson(params):
    return huff_pearson(df, cbg,params[0], params[1])
...
PSO_results = {'cbg':[], 'cost':[], params_dict['p_2']: [], params_dict['p_6']: []}
comm = MPI.COMM_WORLD
size = comm.Get_size()
rank = comm.Get_rank()
...
'''
#r_walks = []
fields=['cbg','cost','R_Percentage_of_Visits_by_brand', 'G_Distance_between_cbg_and_store']
with open('/home/yilunx/Desktop/new_PSO_20220325/calibrations/G_R/sub_' + str(year) +'/PSO_2params_NYC_G_R_sub_' +str(rank) + '_' + str(year) + '.csv', 'a') as myfile:
