...
params_dict = {'p_1': 'I_Includes_parking_lot', 'p_6':'G_Distance_between_cbg_and_store'}
...
def huff_pearson(df, cbg, p_1,p_6):
    rate = get_rate(df, cbg)
    df_temp = df[df['A_cbg'] == cbg].drop(columns = ['A_cbg'])
    df_temp['assumed_rate'] = df_temp.apply(lambda row: compute_row(row,p_1,p_6),axis = 1)
...
PSO_results = {'cbg':[], 'cost':[], params_dict['p_1']:[], params_dict['p_6']: []}
comm = MPI.COMM_WORLD
size = comm.Get_size()
rank = comm.Get_rank()
...
'''
#r_walks = []
fields=['cbg','cost','I_Includes_parking_lot', 'G_Distance_between_cbg_and_store']
with open('/home/yilunx/Desktop/new_PSO_20220325/calibrations/G_I/sub_' + str(year) +'/PSO_2params_NYC_G_I_sub_' +str(rank) + '_' + str(year) + '.csv', 'a') as myfile:
