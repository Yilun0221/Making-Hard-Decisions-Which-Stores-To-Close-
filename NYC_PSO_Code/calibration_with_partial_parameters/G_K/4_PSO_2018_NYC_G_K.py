...
params_dict = {'p_4': 'K_POI_diversity_where_store_is', 'p_6':'G_Distance_between_cbg_and_store'}
...
def compute_row(row,p_4,p_6):
    ...
    up_second = row['K_POI_diversity_where_store_is']**p_4
    down = row['G_Distance_between_cbg_and_store']**p_6
    #return up_first*up_second/down
    return up_second/down

def huff_pearson(df, cbg, p_4,p_6):
    rate = get_rate(df, cbg)
    df_temp = df[df['A_cbg'] == cbg].drop(columns = ['A_cbg'])
    df_temp['assumed_rate'] = df_temp.apply(lambda row: compute_row(row,p_4,p_6),axis = 1)
...
PSO_results = {'cbg':[], 'cost':[], params_dict['p_4']: [], params_dict['p_6']: []}
comm = MPI.COMM_WORLD
size = comm.Get_size()
rank = comm.Get_rank()
...
'''
#r_walks = []
fields=['cbg','cost','K_POI_diversity_where_store_is', 'G_Distance_between_cbg_and_store']
with open('/home/yilunx/Desktop/new_PSO_20220325/calibrations/G_K/sub_' + str(year) +'/PSO_2params_NYC_G_K_sub_' +str(rank) + '_' + str(year) + '.csv', 'a') as myfile:
